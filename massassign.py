...
class PeakPicker:
    def __init__(self):
        ...
        self.selected_peaks = []
        self.mz_values = []
        self.peak_data = []  # List of tuples (t, M/Z, line)
        ...
        self.root = tk.Tk()
        self.add_lines_mode = True  # Mode to add lines
        self.vertical_lines = []  # Store vertical lines
        self.previous_lines = []  # Store previous vertical lines
        ...
        if file_path:
            ...
            for line in lines:
                if line.startswith("TOF"):
                    ...
                    range_start = parts[3].split(': ')[1]
                    range_end = parts[4].split(': ')[1]
                    data.append([t_value, mz_value, area_value, range_start, range_end,self.background_value])
            df = pd.DataFrame(data, columns=['TOF', 'M/Z', 'Area', 'Range Start', 'Range End',"Background"])
            ...
            with pd.ExcelWriter(file_path, engine='xlsxwriter') as writer:
                ...
                worksheet.write('F2', self.background_value)
    ...
    def subtract_count(self):
        try:
            ...
            self.background_value = subtract_value
    ...
    def zoom_out(self):
        self.window_size = min(len(self.tof), self.window_size * 1.5)
        self.zoom_plot()
    ...
    def calculate_area_between_lines(self):
        ...
        for t, mz in relevant_peaks:
            result += f"TOF: {t:.2f}, M/Z: {mz}, Area: {area:.2f}, Range Start: {tof_start:.2f}, Range End: {tof_end:.2f}\n"
        if not self.add_lines_mode:
