...
class PeakPicker:
    ...
    def initialize_variables(self):
        ...
        self.offsets = [0] * 10
        self.shift_amount = 5  # 移動幅の初期値を5に設定
        self.selected_peaks = []
        self.mz_values = []
        self.peak_data = []
    ...
    def create_widgets(self):
        ...
        # ファイル選択ボタン
        self.select_files_button = tk.Button(self.root, text="Select Files", width=15, height=2, command=self.select_files)
        self.select_files_button.grid(row=0, column=0, sticky="nw")

        # 計算結果保存ボタン
        self.save_results_button = tk.Button(self.root, text="Save Results", width=15, height=2, command=self.save_results)
        self.save_results_button.grid(row=0, column=1, sticky="nw")

        # プログラム終了ボタン
        self.quit_button = tk.Button(self.root, text="Quit", width=15, height=2, command=self.root.quit)
        ...
        for i in range(10):
            shift_left_button = tk.Button(self.root, text=f"← {i+1}", width=5, height=1)
            shift_left_button.grid(row=2+i, column=10, sticky="e")
            ...
            shift_right_button = tk.Button(self.root, text=f"{i+1} →", width=5, height=1)
            shift_right_button.grid(row=2+i, column=11, sticky="w")
            ...
            operation_entry = tk.Entry(self.root, width=10)
            operation_entry.grid(row=2+i, column=12, sticky="e")
            self.operation_entries.append(operation_entry)

            apply_button = tk.Button(self.root, text="Apply", width=5, height=1, command=lambda i=i, operation_entry=operation_entry: self.apply_operation(i, operation_entry))
            apply_button.grid(row=2+i, column=13, sticky="w")
            self.apply_buttons.append(apply_button)

            sub_entry = tk.Entry(self.root, width=5)
            sub_entry.grid(row=2+i, column=14, sticky="e")
            self.subtract_entries.append(sub_entry)

            subtract_button = tk.Button(self.root, text="Subtract", width=5, height=1, command=lambda i=i, sub_entry=sub_entry: self.subtract_intensity(i, sub_entry))
            subtract_button.grid(row=2+i, column=15, sticky="w")
            self.subtract_buttons.append(subtract_button)

        # その他の操作ボタン
        self.zoom_in_button = tk.Button(self.root, text="Zoom In", width=15, height=2, command=self.zoom_in)
        self.zoom_in_button.grid(row=11, column=0, sticky="ew")

        self.zoom_out_button = tk.Button(self.root, text="Zoom Out", width=15, height=2, command=self.zoom_out)
        self.zoom_out_button.grid(row=11, column=1, sticky="ew")

        self.full_view_button = tk.Button(self.root, text="Full View", width=15, height=2, command=self.full_view)
        self.full_view_button.grid(row=11, column=2, sticky="ew")

        self.select_button = tk.Button(self.root, text="Select Line", width=15, height=2, command=self.select_line)
        self.select_button.grid(row=11, column=3, sticky="ew")

        self.delete_button = tk.Button(self.root, text="Delete Line", width=15, height=2, command=self.delete_selected_line)
        self.delete_button.grid(row=11, column=4, sticky="ew")

        self.next_button = tk.Button(self.root, text="Next Line", width=15, height=2, command=self.select_next_line)
        self.next_button.grid(row=11, column=5, sticky="ew")

        self.stop_button = tk.Button(self.root, text="Stop Moving", width=15, height=2, command=self.stop_moving_mode)
        self.stop_button.grid(row=11, column=6, sticky="ew")

        self.fit_button = tk.Button(self.root, text="Fit and Calculate", width=15, height=2, command=self.fit_and_calculate)
        self.fit_button.grid(row=11, column=7, sticky="ew")

        self.end_add_lines_button = tk.Button(self.root, text="End Input", width=15, height=2, command=self.end_add_lines)
        self.end_add_lines_button.grid(row=11, column=8, sticky="ew")

        # 移動の幅調整ボタン
        self.increase_shift_button = tk.Button(self.root, text="Increase Shift", width=15, height=2, command=self.increase_shift)
        self.increase_shift_button.grid(row=12, column=0, sticky="ew")
        self.decrease_shift_button = tk.Button(self.root, text="Decrease Shift", width=15, height=2, command=self.decrease_shift)
        ...
        # プロットタイプ変更ボタン
        self.toggle_plot_button = tk.Button(self.root, text="Toggle Plot (TOF/MZ)", width=15, height=2, command=self.toggle_plot)
    ...
    def perform_shift(self):
        if self.shift_active:
            ...
            self.zoom_plot()
            self.root.after(100, self.perform_shift)
    ...
    def apply_operation(self, index, entry):
        ...
        if operation == "":
            if self.saved_intensities[index] is not None:
                self.intensity[index] = self.saved_intensities[index].copy()  # end_add_linesで保存された演算結果に戻す
            else:
                self.intensity[index] = self.base_intensity[index].copy()  # 積算モードの初期値に戻す
        else:
            try:
                result = self.saved_intensities[index].copy() if self.saved_intensities[index] is not None else self.base_intensity[index].copy()
                operations = operation.split('*')
                for op in operations:
                    if '/' in op:
                        numerator, denominator = map(float, op.split('/'))
                        result = (result * numerator) / denominator
                    else:
                        result = result * float(op)
                self.intensity[index] = result
            except ValueError:
    ...
    def subtract_intensity(self, index, entry):
        try:
            ...
            if self.saved_intensities[index] is not None:
                ...
                self.intensity[index] = self.base_intensity[index] - value  # 積算モードの初期値から減算
            self.update_plot()
        except ValueError:
    ...
    def shift_left(self, index):
        self.offsets[index] -= self.shift_amount
        self.tof[index] = self.original_tof[index] + self.offsets[index]
        self.zoom_plot()
