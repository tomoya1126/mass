...
class PeakPicker:
    ...
    def initialize_variables(self):
        self.tof = [np.array([]) for _ in range(10)]
        self.intensity = [np.array([]) for _ in range(10)]
        self.original_tof = [np.array([]) for _ in range(10)]
        self.original_intensity = [np.array([]) for _ in range(10)]
        self.base_intensity = [np.array([]) for _ in range(10)]  # 積算モード変更時の初期値を保存
        self.offsets = [0] * 10
        ...
        self.saved_intensities = [np.array([]) for _ in range(10)]  # end_add_linesで保存される演算結果
        self.last_saved_intensities = [np.array([]) for _ in range(10)]  # end_add_lines前に保存された演算結果
    ...
    def reset_data(self, index):
        self.tof[index] = np.array([])
        self.intensity[index] = np.array([])
        self.original_tof[index] = np.array([])
        self.original_intensity[index] = np.array([])
        self.base_intensity[index] = np.array([])  # 積算モードの初期値をリセット
        self.saved_intensities[index] = np.array([])  # 保存された演算結果をリセット
        self.last_saved_intensities[index] = np.array([])  # 保存された演算結果をリセット
    ...
    def load_data(self, file_path, index):
        ...
        self.tof[index] = data['TOF'].values
        self.intensity[index] = data['Intensity'].values
    ...
    def apply_operation(self, index, entry):
        ...
        if operation == "":
            ...
            elif self.saved_intensities[index].size > 0:
            ...
            try:
                if self.input_mode:
                    ...
                    x = self.saved_intensities[index].copy() if self.saved_intensities[index].size > 0 else self.base_intensity[index].copy()
        ...
        if event.inaxes and event.button == 1:
            ...
            if self.add_lines_mode:
                ...
                if mz_value is not None:
                    ...
                    self.ax.text(peak_time, max(max(intensity) for intensity in self.intensity if intensity.size > 0) * 0.95, f"{mz_value}", color='b', ha='center')
    ...
    def get_closest_mz(self, tof):
        ...
        area = np.sum(self.intensity[0][(self.tof[0] >= closest_tof) & (self.tof[0] <= tof)])
    ...
    def zoom_plot(self):
        ...
        for i in range(len(self.file_paths)):
            if self.tof[i].size > 0 and self.intensity[i].size > 0:
        ...
        for i, (t, mz, line) in enumerate(self.peak_data):
            ...
            if self.plot_type == "TOF":
                ...
                self.ax.text(t, max(max(intensity) for intensity in self.intensity if intensity.size > 0) * 0.95, f"{mz}", color='b', ha='center')
            else:
                if self.fit_done:
                    ...
                    self.ax.text(mz_value, max(max(intensity) for intensity in self.intensity if intensity.size > 0) * 0.95, f"{mz}", color='b', ha='center')
        ...
        self.ax.set_ylim(0, max(max(intensity[(self.tof[i] >= self.center_tof - self.window_size) & (self.tof[i] <= self.center_tof + self.window_size)]) for i, intensity in enumerate(self.intensity) if intensity.size > 0) * 1.1)
        self.canvas.draw()
    ...
    def full_view(self):
        ...
        if self.plot_type == "TOF":
            self.ax.set_xlim(min(min(tof) for tof in self.tof if tof.size > 0), max(max(tof) for tof in self.tof if tof.size > 0))
        else:
            if self.fit_done:
                mz_values = (np.array(self.tof[0]) - self.calculated_tof_mz[0][1]) ** 2 / self.calculated_tof_mz[0][0] ** 2
                self.ax.set_xlim(min(mz_values), max(mz_values))
        self.ax.set_ylim(0, max(max(intensity) for intensity in self.intensity if intensity.size > 0) * 1.1)
        self.canvas.draw()
    ...
    def fit_and_calculate(self):
        ...
        try:
            ...
            max_tof = max(max(self.tof[i]) for i in range(len(self.file_paths)) if self.tof[i].size > 0)
    ...
    def zoom_out(self):
        self.window_size = min(max(len(self.tof[i]) for i in range(len(self.file_paths)) if self.tof[i].size > 0), self.window_size * 1.5)
        self.zoom_plot()
    ...
    def end_add_lines(self):
        ...
        # 各データセットの演算結果を保存
        for i in range(len(self.file_paths)):
            if self.intensity[i].size > 0:
                self.last_saved_intensities[i] = self.saved_intensities[i].copy() if self.saved_intensities[i].size > 0 else self.base_intensity[i].copy()
